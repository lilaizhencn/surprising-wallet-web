...
export default function WalletChainsPage() {
  ...
  const create = async () => {
    ...
    try {
      ...
      await message.success('Chain profile created');
      setOpen(false);
      form.resetFields();
      navigate(`/platform/wallet-config/chains/${result.chain.id}`);
    } catch (error) {
      void message.error(error instanceof Error ? error.message : 'Unable to create chain');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Chains & RPC"
        description="Manage network profiles and open a chain to configure its RPC nodes."
        actions={<Space><Button icon={<ReloadOutlined />} onClick={query.refetch}>Reload</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>Add chain</Button></Space>}
      />
      <ErrorState message={query.error} onRetry={query.refetch} />
      <section className="data-panel">
        <Space wrap className="table-toolbar">
          <Input prefix={<SearchOutlined />} placeholder="Search chain, network or family" value={search} onChange={(event) => setSearch(event.target.value)} allowClear />
          <Space><Switch checked={enabledOnly} onChange={setEnabledOnly} /> Enabled only</Space>
        </Space>
        <Table<WalletChain>
          rowKey="id"
          ...
          columns={[
            { title: 'Chain', render: (_, row) => <><Typography.Text strong>{row.chain}</Typography.Text><br /><Typography.Text type="secondary">{row.nativeSymbol}</Typography.Text></> },
            ...
            { title: 'Task switches', render: (_, row) => [row.scanEnabled && 'Scan', row.withdrawEnabled && 'Withdraw', row.collectionEnabled && 'Collect', row.transferEnabled && 'Transfer'].filter(Boolean).join(' · ') || '—' },
            { title: 'Tokens', dataIndex: 'tokenCount' },
            { title: 'RPC nodes', dataIndex: 'rpcCount' },
          ]}
        />
      </section>
      <Modal title="Add chain profile" open={open} width={900} confirmLoading={saving} onOk={create} onCancel={() => setOpen(false)} destroyOnHidden>
