...
export default function WalletKeysPage() {
  ...
  const save = async (values: WalletKeysetInput) => {
    ...
    try {
      ...
      await message.success('Wallet keyset saved');
      query.refetch();
    } catch (error) {
      void message.error(error instanceof Error ? error.message : 'Unable to save wallet keyset');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="Wallet keys"
        description="Manage the four seeds as one atomic wallet keyset."
      ...
      <Alert
        type="warning"
        showIcon
        title="Development-only plaintext storage"
        description="All four values are stored as plaintext in PostgreSQL and returned by this page. Each value must be Base64 and decode to exactly 32 bytes. Start or restart sig1 and sig2 after saving so their JVMs load the keyset."
      />
      {query.data?.locked ? (
        <Alert
          type="info"
          showIcon
          title="Keyset locked"
          description="Derived addresses already exist, so the keyset can be viewed but cannot be changed."
        />
      ) : null}
          ...
          <>
            <Space wrap className="wallet-key-status">
              <Tag color={query.data?.configured ? 'green' : 'gold'}>
                {query.data?.configured ? 'Configured' : 'Not configured'}
              </Tag>
              {query.data?.updatedAt ? (
                <Typography.Text type="secondary">
                  Updated {formatDate(query.data.updatedAt)} by {query.data.updatedBy ?? 'unknown'}
                </Typography.Text>
              ) : null}
            </Space>
                      ...
                      validator: (_, value: string) => {
                        try {
                          ...
                          return bytes.length === 32
                            ? Promise.resolve()
                            : Promise.reject(new Error('Must decode to exactly 32 bytes'));
                        } catch {
                          return Promise.reject(new Error('Must be valid Base64'));
                        }
                      },
                    },
                  ]}
                >
                  <Input.Password visibilityToggle autoComplete="off" placeholder="Base64-encoded 32-byte seed" />
                </Form.Item>
              ))}
              <Button
                type="primary"
